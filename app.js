...
function handleLoginSubmit() {
  ...
  // ── Look up account by email only – role is NOT supplied by the user ──────
  ...
  // ── Credential check (supports legacy accounts without a stored password) ─
  ...
  // ── Establish secure session – role is READ from the database record ──────
  currentSession = Object.assign({}, user);   // shallow clone: mutations stay isolated
  ...
  if (user.role === 'Student')        checkAuthAndNavigate('student-view');
    ...
    // Fallback – unknown role lands on catalog (safe default)

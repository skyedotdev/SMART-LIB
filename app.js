    ...
    row.innerHTML = `
      <div style="flex:1;min-width:0;">
        <div style="display:flex;align-items:center;gap:.6rem;flex-wrap:wrap;margin-bottom:.4rem;">
          <p style="font-weight:700;color:var(--text-dark);font-size:.9rem;margin:0;">${escapeHTML(book.title)}</p>
          <span class="genre-tag">${escapeHTML(genreDisplay)}</span>
          <span class="badge-tag ${book.mediaClass==='DIGITAL'?'tag-digital':'tag-physical'}" style="margin-bottom:0;">${book.mediaClass==='DIGITAL'?'Digital':'Physical'}</span>
          <span class="availability-pill ${book.available?'avail-yes':'avail-no'}" style="position:static;">${book.available?'Available':'On Loan'}</span>
        </div>
        <div class="book-detail-panel">
          ...
          <div class="book-detail-field">
            ...
            <div class="book-detail-value mono">${escapeHTML(book.isbn || '—')}</div>
          </div>
          <div class="book-detail-field">
            <div class="book-detail-label">Publisher</div>
            <div class="book-detail-value">${escapeHTML(book.publisher || '—')}</div>
          </div>
          <div class="book-detail-field">
            <div class="book-detail-label">Year Published</div>
            <div class="book-detail-value">${escapeHTML(book.pubDate || '—')}</div>
          </div>
          <div class="book-detail-field">
            <div class="book-detail-label">Genre</div>
            <div class="book-detail-value">${escapeHTML(genreDisplay)}</div>
          </div>
          <div class="book-detail-field">
            <div class="book-detail-label">Language</div>
            <div class="book-detail-value">${escapeHTML(book.language || 'English')}</div>
          </div>
          <div class="book-detail-field">
            <div class="book-detail-label">Media Class</div>
            <div class="book-detail-value">${escapeHTML(book.mediaClass || '—')}</div>
          </div>
          <div class="book-detail-field">
            <div class="book-detail-label">Rate Class</div>
            <div class="book-detail-value">${book.rateClass === 'HIGH_PRIORITY' ? 'High-Priority ($2.50/day)' : 'Standard ($1.00/day)'}</div>
          </div>
          ${book.description ? `
          <div class="book-detail-field book-detail-full">
            <div class="book-detail-label">Description</div>
            <div class="book-detail-value">${escapeHTML(book.description)}</div>
          </div>` : ''}
        </div>
      </div>
      <button onclick="openEditBookModal(${book.id})" class="btn btn-blue" style="font-size:.72rem;padding:.35rem .75rem;flex-shrink:0;align-self:flex-start;gap:.35rem;">${icon('edit')} Edit</button>`;
    el.appendChild(row);
  });
}

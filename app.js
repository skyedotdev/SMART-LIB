...
// ── SEED CATALOG (runs once if the catalog has never been populated) ─────────
// Each entry follows strict metadata rules:
//   genre   → single descriptive word/phrase only (no full sentences)
//   pubDate → 4-digit year only
//   isbn    → used to auto-fetch cover from Open Library Covers API
...
function handleLoginSubmit() {
  ...
  if (!isValidUnivEmail(email)) {
    showAlert('Invalid email: Only university email addresses are accepted (e.g. username@univ.edu).', 'error');
    return;
  }
  if (!password) {
  ...
  if (!user) {
    showAlert('CAS Rejection: No account found for this email. Please register first.', 'error');
    return;
  }

  // Support legacy accounts created before passwords were added
  ...
  // Establish session — role is READ from the stored account record
  ...
  if (user.role === 'Student')        checkAuthAndNavigate('student-view');
    ...
    showAlert('Your account role is unrecognised. Contact the administrator.', 'error');
  }
}
...
function filterCatalog() {
  ...
  books.forEach(book => {
    ...
    // Build Open Library cover URL; fall back to a neutral placeholder if ISBN is absent
    ...
    const coverImgHTML = coverSrc
      ? `<img
           ...
           loading="lazy"
           onerror="this.onerror=null;this.src='';this.closest('.book-cover-wrap').classList.add('cover-fallback');"
         >`
      : '';
